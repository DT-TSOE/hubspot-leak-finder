      ...
      {sources.map((s,i)=>(
        <div key={s.source} style={{ marginBottom:9 }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
            ...
            <span style={{ fontSize:13, color:'#999' }}>{s.won}/{s.total} · <span style={{ fontWeight:600, color:'#243A52' }}>{s.winRate}%</span></span>
          </div>
          <div style={{ height:7, borderRadius:4, background:'#F3F4F6', overflow:'hidden' }}>
            <div style={{ height:'100%', width:`${(s.winRate/max)*100}%`, background:i===0?'#0091AE':'#CBD6E2', borderRadius:4, transition:'width .5s' }}/>
          </div>
        </div>
      ))}
    </div>
